main(){
  ...
  if [ ! -d "$DEVICE_DIRECTORY" ]; then
    ...
    # REPLACE THE MOUSTACHE TAGS WITH THE REQUIRED INFORMATION -- For BSD (and OSX) sed variants
    ...
    # REPLACE THE MOUSTACHE TAGS WITH THE REQUIRED INFORMATION -- For GNU sed variants 
    sed -i.sed "s|{{skin_name}}|$DEVICE|" $CONFIG_TEMPLATE_FILE.tmp
    sed -i.sed "s|{{skin_path}}|$ANDROID_SKINS_DIRECTORY/$DEVICE|" $CONFIG_TEMPLATE_FILE.tmp
